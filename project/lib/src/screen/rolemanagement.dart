...
Future<void> deletepaymentid(Order order) async{
 final url = Uri.parse('http://$ip:3000/tradetryst/deletepayorder/paydelete/${order.paymentId}'); 
...
// payment id for order
  ...
  Future<void> getproductdrompay() async {
    ...
    final url = Uri.parse('http://$ip:3000/tradetryst/orderproduct/productlistt'); // replace with your API endpoint
    final response = await http.get(
      url,
      headers: <String, String>{
        'Content-Type': 'application/json; charset=UTF-8',
       ...
       appBar: PreferredSize(
        ...
        child: Container(
          ...
          child: AppBar(
            ...
            actions: <Widget>[
              IconButton(
               ...
               icon: Icon(Icons.bar_chart, size: 28), // Add this icon
                  onPressed: () {
                   Get.to(() => StatisticsPage()); // Navigate to the statistics page
                  },
  ),
            ],
            ...
            backgroundColor: Colors.transparent, // Make AppBar transparent to show Container's decoration
